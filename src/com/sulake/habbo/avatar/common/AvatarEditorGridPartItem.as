package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_832:IFigurePartSet;
      ...
      private var var_664:Rectangle;
      
      private var var_2004:int = 0;
      
      private var var_460:IAvatarRenderManager;
      
      private var var_1091:Boolean;
      
      private var var_2006:BitmapData;
      
      private const const_2194:int = 13421772;
      
      private const const_2193:int = 16777215;
      
      private const const_376:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 == null)
         {
            this.var_714 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_2004 = Math.max(this.var_2004,_loc6_.colorLayerIndex);
            }
         }
         this.var_460 = this.var_66.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_714 != null && !this.var_2005)
            {
               _loc3_ = this.var_714;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_832)
            {
               switch(this.var_832.clubLevel)
               {
                  case HabboClubLevelEnum.const_56:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_45:
                     _loc2_.style = HabboIconType.const_808;
                     break;
                  case HabboClubLevelEnum.const_46:
                     _loc2_.style = HabboIconType.const_947;
               }
            }
            else
            {
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_460.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_460.isFigureReady(_loc1_))
         {
            this.var_460.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_376[_loc6_] + "_" + FigureData.const_153;
               _loc4_ = this.var_460.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_376.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_376[_loc6_] + "_" + FigureData.const_153;
                  _loc4_ = this.var_460.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_664 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_2006)
               {
                  ...
                  this.var_2006 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_664.width,this.var_664.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_664.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_664.y;
               if(this.var_2005 && _loc2_.colorLayerIndex > 0)
               {
